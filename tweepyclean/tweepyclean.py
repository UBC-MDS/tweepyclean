...
def clean_tweets(tweets_df, handle = "", text_only = True, word_count = True, emojis = True, hashtags = True, sentiment = True, flesch_readability = True, media_links = True, proportion_of_avg_retweets = True, proportion_of_avg_favorites = True):    
    """
    ...
    proportion_of_avg_favorites: bool, optional
        Bool which specifies to add a column containing a proportion value of how many favorites a tweet received compared to the account average (default is True)
    ...
    if not 'retweet_count' in tweets_df.columns:
        raise ValueError("input dataframe does not contain retweet_count column needed to generate proportion_of_avg_retweets")
    if not 'favorite_count' in tweets_df.columns:
        raise ValueError("input dataframe does not contain favorite_count column needed to generate proportion_of_avg_favorites")
    if not 'entities' in tweets_df.columns:
        raise ValueError("input dataframe does not contain entities column needed to generate media_links or hashtags")
    if not 'full_text' in tweets_df.columns:
        raise ValueError("input dataframe does not contain full_text column needed to generate text_only and sentiment")
    ...
    if (proportion_of_avg_retweets == True):
        avg_retweets = tweets_df_new["retweet_count"].mean()
    if (proportion_of_avg_favorites == True):
        avg_favorites = tweets_df_new["favorite_count"].mean()

    for i in range(len(tweets_df_new)): # Loop to run through each tweet

        entity = tweets_df_new.at[i,'entities'] # extract entities column containing hashtags and other information.
        tweet_text = tweets_df_new.at[i,'full_text'] # Get text of current tweet

        # Text only extraction
        text_only_col = re.sub(r"\$\w+[,]|\@\w+|[,]\$\w+", "", tweet_text) # removes '@'s
        text_only_col = re.sub(r"\$\w+[,]|\#\w+|[,]\$\w+", "", text_only_col) # removes hashtags
        text_only_col = re.sub(r'http\S+', '', text_only_col) # removes https links
        ...
        text_only_col = text_only_col.lstrip().rstrip() # lstrip removes leading whitespace, rstrip trailing
        ...
        # Wordcount
        if (word_count == True):
            tweets_df_new.at[i,'word_count'] = len(text_only_col.split()) # New column

        # Emoji extraction
        if (emojis == True):
            emoji_list = []
            emoji_list = re.findall(emoji.get_emoji_regexp(), tweet_text) # Extracts hashtags and puts them in dict
        ...
        if (hashtags == True):
            hashtags = []
            hashtags = re.findall(".*?\\s(#\\w+).*?", tweet_text) # Extracts hashtags and puts them in dict
        ...
        if (sentiment == True):
            tweets_df_new.at[i, 'sentiment_polarity'] = mod.polarity_scores(str(text_only_col))['compound'] # New column

        # flesch_readability
        if (flesch_readability == True):
            tweets_df_new.at[i, 'flesch_readability_score'] = textstat.flesch_reading_ease(text_only_col) # New column

        # media_links
        # if (media_links == True):
        #     if 'media' in entity:
        #         if 'type' in entity['media'][0]:
        #             tweets_df_new.at[i, 'media'] = "yes"
        #             tweets_df_new.at[i, 'media_url'] = entity['media'][0]['media_url_https']
        #             tweets_df_new.at[i, 'media_link'] = entity['media'][0]['url']
        #     else:
        #         tweets_df_new.at[i, 'media'] = "None"
        #         tweets_df_new.at[i, 'media_url'] = "None"
        #         tweets_df_new.at[i, 'media_link'] = "None"

        # proportion_of_avg_retweets
        if (proportion_of_avg_retweets == True):
            tweets_df_new.at[i, 'prptn_rts_vs_avg'] = tweets_df_new.at[i,'retweet_count'] / avg_retweets # New column
